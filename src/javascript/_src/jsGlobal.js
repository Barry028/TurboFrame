...
(function() {
    ...
    function stringExtend() {
        ...
        String.prototype.dashString = function() {
            // 這裡面的this實際上指向的就是我們自己定義的一個變量字符串
        ...
        String.prototype.reverse = function() {
             ...
             */
        String.prototype.unescapeHTML = function() {
                // 由於這裡的 this 實際上拿到的是一個字符串數組, 因此第一步需要先把字符串數組轉換為一個字符串
                console.log(typeof this);
                // 1.先把這個偽數組轉換為數組對象
                var arr = Array.prototype.slice.call(this);
                // 2.把數組中的內容轉換為字符串
    ...
    function arrayExtend() {
        ...
        Array.prototype.avg = function() {
            ...
            /*return cacl(this, function (item, avg) {
                // 1. 先求和(進入到這個函數裡面， this指向的是window對象，此時window對象是沒有sum方法的，故執行錯誤)
                //var sum = this.sum();
                // 2.求出平均值
        ...
        Array.prototype.unique = function() {
            ...
            for (var i = 0; i < len; i++) {
                ...
                // 由於tmp.length初始的位置一直是0， 添加一個元素之後變為1，因此下標和長度每次相差1， 實現了實時插入數據的功能
        ...
        Array.prototype.map = function(fn, ctx) {
                ...
                for (; i < len; i++) {
                    ...
                    // 將每一項執行的結果放入到一個新的數組裡面
                    ret.push(res);
                }
                return ret;
            }
            /**
             * 對數組的每一項執行回調函數， 返回回調函數執行結果為true的數組集合
             * @ Param fn
             * @ Param ctx
             */
        Array.prototype.filter = function(fn, ctx) {
            ...
            for (; i < len; i++) {
                // 注意這裡的這種運算方式只會返回所有的回調函數返回true的計算結果集
        ...
        Array.prototype.every = function(fn, ctx) {
                var i = 0,
                    len = this.length;
                // 遍歷數組中所有的元素， 只要有一個函數回調函數為false就返回false,只要所有的都是true才會返回true
                for (; i < len; i++) {
                    // 如：a默認是undefined，!a是true，!!a則是false，所以b的值是false，而不再是undefined。這樣寫可以方便後續判斷使用。
                    // 所以，!!(a)的作用是將a強制轉換為布爾型（boolean）。
                    // 如果a = null, !!(a) 的結果就是假， 可以直接把一個弱類型強制轉換為一個新的類型
                    // 下面的代碼就是強制將一個函數轉換為bool的類型
            ...
            /**
             * 對數組中的每一項執行回調函數，只要有一項為true的話，就是true，否則就是false
             * @ Param fn
             * @ Param ctx
             */
        ...
        Array.prototype.reduce = function(callback) {
            var i = 0,
                len = this.length,
                callbackRet = this[0]; // 這個變量保存著上一次回到的函數的返回結果， 默認存儲的是第一個元素
            for (; i < len; i++) {
                // this的指向，element， index， 數組對象本身
                // callbackRet 裡面存儲了數組上一次計算的處理結果
        ...
        Array.prototype.reduceRight = function(callback) {
            var len = this.length,
                i = this[len - 2],
                callbackRet = this[len - 1]; // 保存著最後一項
        ...
        Array.prototype.indexOf = function(target, start) {
            ...
            * */
            var len = this.length,
                start = ~~start; // 如果start不傳過來，這裡就是undefined，指向後面的就會保存，這裡使用了~~把其他類型強制轉換為數字類型
            if (start < 0) {
        ...
        Array.prototype.enhanceUnique = function() {
            ...
            for (; i < len; i++) {
                ...
                if (tempMap[temp] === void 0) {
                    ret.push(temp);
                    // {}數據的存儲格式為{1 : true, 2 : false, 3 : false}
                    tempMap[temp] = true;
                }
            }
            return ret;
        ...
        /**
         * 刪除數組中的指定元素， 通過arguments偽數組的方式來接受傳遞過來的參數
        ...
        Array.prototype.without = function() {
            ...
            * */
            //1. 由於arguments實際上是一個偽數組，不能直接使用數組裡面的方法
            // 因此先要把arguments轉換為數組
            ...
            // 遍歷原始的數組(由於後面每次刪除掉一個元素之後，這裡的this.length的長度就是已經都改變了， 因此每次在執行完畢之後都要重新計算一下length)
            for (; i < len; i++) {
                for (; j < aLength; j++) {
                    if (this[i] === args[j]) {
                        // 只要刪除的數組在我的這個裡面，就直接去掉
                        // i 為起始的值，1為要刪除的項， 也就是刪除i位置的元素
                        // splice  返回的是刪除的元素， this內容是已經修改過之後的項
                        this.splice(i, 1);

                        // 為了避免刪除數組的元素之後的數組長度的變化，這裡需要重新計算一下數組的新的長度
                        // len = this.length;
                    }
        ...
        /**
         * 實現一個數組的扁平化(可以解決數組裡面存放數組的問題)【遞歸處理調用】
         * [[], [], [], [[], [], []]]
         * @ Return {Array}
         */
        Array.prototype.flatten = function() {
            ...
            // 數組中元素僅基本類型的元素或數組，
            var ret = [],
                len = this.length, // 注意當下一次執行遞歸調用之後，這裡的this指向的是tmp
                i = 0,
                tmp;
            ...
            for (; i < len; i++) {
                // 注意這裡先取出來數組中的每一項元素
                tmp = this[i];
                // 判斷一下數組裡面存放的還是不是數組類型（數組裡面的每一項）
                if (({}).toString.call(tmp) === '[object Array]' || Object.prototype.toString.call(tmp) === '[object Array]') {
                    // 繼續遞歸調用(遞歸調用的時候需要把結果存起來哦)
                    // 1. 對當前數組裡面的數組進行扁平化處理, tmp.flatten()得到的就是一個普通的數組類型
                    // 2. 由於ret是一個數組類型，使用concat之後可以把兩個數組裡面的元素鏈接起來
                    // 下一次執行遞歸的時候上面的this就是指向了這裡的tmp數組
                    ret = ret.concat(tmp.flatten())
                        //tmp.flatten();
                } else {
                    // 如果不是數組類型的話，就直接放入到我的新數組裡面
                    ret.push(tmp);
                }
            }
            return ret;
        ...
        【經驗話語1】
          ...
          不管變量有沒有聲明，都可用typeof 判斷，注意typeof 返回結果為字符串，所以是與"undefined"做比較。
          所以，判斷類型最好用typeof ，因為當判斷的變量是在其他js 文件中定義的全局變量時，
